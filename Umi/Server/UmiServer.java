...
public class UmiServer {
	...
	public static void big(String name){
		SIZE = 40;
	}
       public static void normal(String name){
                SIZE = 20;
        }
       public static void small(String name){
                SIZE = 10;
        }
	// calculationメソッド
					...
					if (r < SIZE) {
						...
						//ship.point++;
							...
							int energy_point = 0;
									...
									energy_point = 10;
						                        System.out.println("▼  " + user + ": E +10");
						                }
						ship.point = ship.point + energy_point;
		...
		if (userTable != null){
			for (Enumeration users = userTable.keys();
				...
				pw.println(user + " " + ship.x + " "
								+ ship.y + " " + String.format("%.1f",ship.point) + " ");//ship.point
			}
		}
		...
		try {
			...
			while (true) {
				...
				if (name == null){
					...
					//
					} else if ("BIG".equalsIgnoreCase(cmd)){
						UmiServer.big(name);
                                        } else if ("NORMAL".equalsIgnoreCase(cmd)){
                                                UmiServer.normal(name);
                                        } else if ("SMALL".equalsIgnoreCase(cmd)){
                                                UmiServer.small(name);
                        		//
