...
public class UmiClient implements Runnable {
	...
	public static double old_point = 0.0;
	// コンストラクタ
	// GUI 画面の初期配置を行います
	public UmiClient ()
	{
	...
	void repaint(){
		...
		try {
			...
			while (!".".equals(line)){
				...
				Double point = Double.valueOf(P);
				int size = 20;
				//double old_point = ;
				// 船を表示します
				if(point.doubleValue() > 10.0){
					g.fillOval(x - 10, 256 - y - 10, 30, 30);
					size = 30;
					sendCommand("Big");
				}else if(point.doubleValue() > 5.0){
					g.fillOval(x - 10, 256 - y - 10, 20, 20);
					size = 20;
					sendCommand("normal");
				}else {
					g.fillOval(x - 10, 256 - y - 10, 10, 10);
					size = 10;
					sendCommand("small");
				}

				// 得点を船の右下に表示します
				g.drawString(P,x + size,256-y+10);//x + 10
